...
class BasicPluginFramework {
    ...
    *       create_nonce()
    *       ==============
    *       A security feature you should use. Please refer to:
	*       http://codex.wordpress.org/WordPress_Nonces
    */
    function create_nonce( $name ) {
        $this->nonce($name) = wp_create_nonce("basicpf-$name-nonce");
	...
	*       The data can be used within the templates when using the "data"-Prefix
    *       A few examples:
    *
    ...
    */
    function render($view) {
        $template_file = $this->plugin_dir.'/views/'.$view.'.tpl';
		if ($data) {
